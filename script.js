...
class BrutalBeerStats {
    ...
    renderQualityMatrixChart() {
        ...
        const data = Object.entries(styleStats)
    ...
    renderSeasonalPatternsChart() {
        ...
        this.data.forEach(d => {
            ...
            if (!monthlyData[month][d.beer_type]) {
                monthlyData[month][d.beer_type] = 0;
            }
            monthlyData[month][d.beer_type]++;
        });
        ...
        const data = Array.from({length: 12}, (_, month) => {
            ...
            topStyles.forEach(style => {
                result[style] = monthlyData[month] ? (monthlyData[month][style] || 0) : 0;
            });
            return result;
        });
        ...
        g.selectAll('.layer')
            ...
            .on('mouseover', function(event, d) {
                ...
                tooltip.html(`
                    <strong>${monthNames[d.data.month]}</strong><br/>
        ...
        g.append('g')
            .attr('transform', `translate(0,${height})`)
            .call(d3.axisBottom(x).tickFormat(d => monthNames[d]))
            .selectAll('text')
            .attr('class', 'chart-axis')
            .style('font-size', '10px')
            .style('font-weight', 'bold');
    ...
    renderWeekendRhythmChart() {
        ...
        this.data.forEach(d => {
            ...
            if (!rhythmData[key]) {
                rhythmData[key] = { day, hour, count: 0 };
            }
        ...
        g.selectAll('.rhythm-cell')
            ...
            .on('mouseover', function(event, d) {
                ...
                tooltip.transition().duration(200).style('opacity', 1);
                tooltip.html(`
                    <strong>${dayNames[d.day]} ${d.hour}:00</strong><br/>
